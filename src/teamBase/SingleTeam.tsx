...
export const SingleTeam = ({ team }: SingleTeamProps) => {
  ...
  return (
    <Item>
      ...
      {mode === "add" && (
        <AddPlayerTeam relation={relation} player={team.players} id={team.id} />
      )}
      ...
      {mode === "remove" && (
        <RemovePlayerTeam relation={relation} player={team.players} id={team.id} />
      )}

      <ActionButton onClick={toggleDeleteMode}>
        {mode === "delete" ? "Cancel" : "Delete"}
      </ActionButton>
      {mode === "delete" && <DeletePlayer team={team} onCancel={toggleDeleteMode} />}
    </Item>
  );
};
